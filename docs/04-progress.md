...
| **M5** | **原生 McMaille 照搬 + 种子精修** | ✅ 完成 | 后处理链路有效；Top-1 44%→46%。⚠️ 所用种子模型属路线偏离产物 |
| **M6** | **Arm A 消融 → 主线回 PXRD-indexer** | 🟡 | 100k 消融否决 RealPXRD 迁移优势；4 卡严口径重训待启动。见 20260729 开发日志 |
| **M4** | **完胜 JADE/Mc 攻关（段 0–3）** | 🟡 | R3–R5 闭环：冠军 `cubic_split_clf` **elem 15.43%（过 Gate）**；ang/非立方/pull90 **未过**；R4/R5 先验·多锚·输入均未超冠军。见 R5-C 定案 |

## 变更日志

### 2026-07-29（Arm A 消融定案 + PXRD-indexer 重训协议）

- 自研线命名 **PXRD-indexer**（`--backbone peaktf`）
- Arm A 100k 消融（freeze/unfreeze/砍CSPNet/Fourier/scratch d256L6）：全部 **40–44%**，
  低于 PXRD-indexer **51%** → **停止抠 Arm A 架构**；CSPNet 死重、解冻无用
- 探针：`sample_steps` 25→400，`<1%` 命中 51%→50% → **不是瓶颈**
- 旧 full6m：未训满 + resume 丢 AdamW 动量；MP100@5% 选模噪声大；L4 虚高约 12pp
- 重训协议：valid `<1%` 选模 / CRN / metrics.csv / DDP global-batch=512 / 从零 / 4 卡火山
- 本轮不做：MAX_PEAKS、体积先验、低对称上采样
- 文档：[`20260729-ArmA消融与PXRD-indexer重训协议.md`](开发日志/20260729-ArmA消融与PXRD-indexer重训协议.md)
- ArmA 6M 对照仍在跑（ep1 library@100=30%）；不挡 indexer 重训

### 2026-07-28（路线偏离纠正）

- 种子生成器实际实现（自研 PeakTransformer + 自研 flow，6M 从头训）
  **偏离** PM 指定的方案 A（RealPXRD 预训练迁移），且变更未报备
- McMaille 后处理成果与种子来源无关，全部保留
- PM 拍板：按方案 A 重做（预训练 Bert + CSPFlow + 重设计 output 头 + 6M）；自研线留作对照
- 说明：[`20260728-路线偏离说明与纠正.md`](开发日志/20260728-路线偏离说明与纠正.md)

### 2026-07-28（M5：照搬原生 McMaille → 精修 → 护栏修复 → 瓶颈定位在生成侧）

主线四步，MP100 · prim · L4-strict（分母 100）：

| 阶段 | 关键动作 | library | Top-1 |
|---|---|---:|---:|
| 非破坏多阶段（原基线） | — | 68% | 35% |
| 1 忠实照搬原生链路 | `--seed-policy native` | 34% | 24% |
| 2 + 度规张量约束 LSQ 精修 | `--refine lsq` | 35% | 26% |
| 3 + 反射数护栏修复 | `MCM_NHKL_CAP=400` | 57% | **44%** |
| 4 + 推理 K=1000 | — | **64%** | **46%** |

- 第 3 步后忠实照搬**首次超过自研多阶段**（44% vs 36%）
- 根因是 `CALCUL1` 里 `IF(NHKL.GT.NDAT10)RETURN` **未给 DIFF 赋值**的未定义行为；
  同一份代码这条路径怎么解释，library 在 28%/35%/49%/57% 之间摆动
- 三次判断被证据推翻：「门控是唯一瓶颈」「SUPCEL/去重/RMAX2 在杀答案」「缺口是误杀」
- **瓶颈已转移到生成侧**：K=1000 池名义 recall 73%，真正堪用（<1%）仅 61%，
  而全流程 library 已 64%——下游把种子池榨干
- 失败分析两条主轴：晶系自由度（cubic 100% → triclinic 0%）、晶胞尺寸（good 169 Å³ vs miss 586 Å³）；
  决定性证据是**体积先验偏小**（miss 组候选体积中位仅真值 0.51 倍，6/27 够不到 90% 体积）
- 已决定：选模换严口径（best error <1%）、当前 `full6m_equiv_off` 停止（ep18/34）、按新设计重开
- 复盘：[`20260728-McMaille照搬到瓶颈定位-完整复盘.md`](开发日志/20260728-McMaille照搬到瓶颈定位-完整复盘.md)
- 实验记录：[照搬与AB](实验记录/20260728-原生McMaille单候选全流程照搬与AB.md)、
  [LSQ精修](实验记录/20260728-度规张量约束最小二乘精修.md)、
  [护栏与门控归因](实验记录/20260728-反射数护栏未定义行为与门控归因.md)、
  [K扩展](实验记录/20260728-推理K扩展100-500-1000.md)、
  [失败分析](实验记录/20260728-种子池失败分析与体积先验偏小.md)
