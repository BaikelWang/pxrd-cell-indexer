#!/usr/bin/env python3
"""M1.4 valid stratified sample: full scan of pxrd_241113_valid.lmdb.

Logic mirrors investigate_10k_sample.py but scans the entire valid LMDB (no pool
stage). Intentionally not refactored into a shared module to avoid touching the
already-verified train10k outputs.

Produces:
  - data/processed/valid1400_seed42.jsonl
  - data/processed/investigate_valid_stats_seed42.json
"""
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    valid_size = len(keys)
    all_indices = list(range(valid_size))
    workers = min(args.workers, cpu_count(), valid_size)

    print(f"Stage A: basic stats on full valid set (n={valid_size}) with workers={workers}")
    with Pool(
        processes=workers,
        ...
        all_records = list(
            tqdm(
                pool.imap(_basic_stats, all_indices, chunksize=256),
                total=valid_size,
                desc="stage_a",
            )
        )

    atom_nums = [r.atom_num for r in all_records]
    peaks_filtered = [r.peak_num_filtered for r in all_records]

    small_pool = [r for r in all_records if r.atom_num < 25]
    print(
    ...
    cs_counter_valid = Counter(r.crystal_system for r in selected)
    peaks_valid = [r.peak_num_filtered for r in selected]
    ...
    stats: dict[str, Any] = {
        ...
        "stage_b": {
            ...
            "crystal_system_counts_in_valid": _counter_to_dict(cs_counter_pool),
        },
        "sample_valid": {
            **sample_meta,
            "crystal_system_counts": _counter_to_dict(cs_counter_valid),
            "peak_num_filtered": _percentiles(peaks_valid, [50, 90, 95, 99]),
        },
    }

    args.output_dir.mkdir(parents=True, exist_ok=True)
    jsonl_path = args.output_dir / f"valid{args.target_size}_seed{args.seed}.jsonl"
    stats_path = args.output_dir / f"investigate_valid_stats_seed{args.seed}.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="M1.4: valid stratified sample + peak stats")
