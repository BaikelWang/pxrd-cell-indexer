#!/usr/bin/env python3
"""M1.4 test stratified sample: full scan of pxrd_241113_test.lmdb.

Logic mirrors investigate_valid_sample.py but scans the held-out test LMDB.

Produces:
  - data/processed/test1400_seed42.jsonl
  - data/processed/investigate_test_stats_seed42.json
"""
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    print(f"Stage A: basic stats on full test set (n={valid_size}) with workers={workers}")
    with Pool(
        processes=workers,
    ...
    stats: dict[str, Any] = {
        ...
        "stage_b": {
            ...
            "crystal_system_counts_in_test": _counter_to_dict(cs_counter_pool),
        },
        "sample_test": {
            **sample_meta,
    ...
    jsonl_path = args.output_dir / f"test{args.target_size}_seed{args.seed}.jsonl"
    stats_path = args.output_dir / f"investigate_test_stats_seed{args.seed}.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="M1.4: test stratified sample + peak stats")
